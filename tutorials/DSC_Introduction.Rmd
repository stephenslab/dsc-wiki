...
identifying the *types of module* and the *quantities that are going to be passed from module to module*. In DSC we refer to quantities that are passed from module to module as "pipeline variables". Pipeline variabes should be given an informative name which must begin with a `$`. 
...
2. after running the script in `mean.R` (or `median.R`) it should set the value of the pipeline variable `$mean_est` to the value of the script variable `y`.
